$(function(){
	...
	function getBoss(get_play,cancelNum){	//get_play是用来接收随机数的，当随机数为0时，就是左边玩家，1就是中间玩家，2就是右边玩家
		...
		if(get_play == 0){	//左边玩家抢地主按钮显示
			$('.play_btn').eq(0).css({'transform':'rotateZ(90deg)','margin-left':'259px','margin-top':'255px'});
		}else if(get_play == 2){	//右边玩家抢地主按钮显示
			$('.play_btn').eq(2).css({'transform':'rotateZ(-90deg)','margin-left':'-270px','margin-top':'280px'});
		}else{	//中间玩家抢地主按钮显示
			$('.play_btn').eq(1).css({'margin-top':'20px'});
		}
				...
				setTimeout(function(){
					...
					for(let i=0;i<player[get_play].poker.length;i++){
						...
						if(get_play == 1){	//中间玩家的放牌方式
							...
							$('.play').eq(get_play).css('top','0px');
						}else if(get_play == 0){	//左边玩家的放牌方式
							$('.play').eq(get_play).append(poker_html);
							$('.play').eq(get_play).find('li:last').css({top:24*i+'px','transform':'rotateZ(90deg)'});	//
							$('.play').eq(get_play).css({top:'50px'});
						}else{	//右边玩家的放牌方式
							$('.play').eq(get_play).append(poker_html);
							$('.play').eq(get_play).find('li:last').css({top:-24*i+'px','transform':'rotateZ(90deg)'});	//
							$('.play').eq(get_play).css({top:'500px'});
						}
					}
					//开始出牌阶段
	...
	function playPoker(index,cancelNum){
		...
		if(index == 0){	//左边玩家出牌按钮显示
			...
			$('.play_btn2').eq(1).css({'margin-top':'20px'});
		}
			...
			if(game.select_poker.type == 0){
				...
				if(vsPoker()){
					...
					if(player[index].poker.length == 0 && player[index].role == 1){
						...
						$('.result').find('li').eq(0).css({display:'block'});
						...
						$('.desktopPoke').eq(index).find('li').remove(); 
						$('.play_btn2').eq(index).remove();
						$('.result').find('li').eq(1).css({display:'block'});
						$('.result button').click(function(){
							window.location.href = window.location.href;
						});
					}
	...
	function tipsPoker(index){
		...
		if(game.desktop_poker.poker.length == 2){
			if(game.desktop_poker.type == 2){//两张是一对的时候
				for(let j=str.length;j>=0;j--){
					$('.play').eq(index).find('li').eq(j).trigger('click');
			...
			}else if(game.desktop_poker.type == 110){//两张是王炸的时候
		...
		if(game.desktop_poker.poker.length == 4){
			if(game.desktop_poker.type == 911){//判断为炸弹
				for(let j=str.length;j>=0;j--){
					$('.play').eq(index).find('li').eq(j).trigger('click');
					$('.play').eq(index).find('li').eq(j-1).trigger('click');
					$('.play').eq(index).find('li').eq(j-2).trigger('click');
	...
	// 	if(game.desktop_poker.poker.length == 0){
	// 		game.select_poker.poker = player[index].poker.pop();
	// 		// game.select_poker.max = player[index].poker[length-1];
	// 		vsPoker();
	// 	}
	// 	console.log(game.select_poker.poker);
		...
		for(let i=0;i<select_poker.length;i++){
			for(let j=0;j<player_poker.length;j++){
				if(select_poker[i].num == player_poker[j].num && select_poker[i].color == player_poker[j].color){
					...
					else if(player[index].poker.length == 0 && player[index].role == 0 && player[index+1].poker.length == 17 && player[index+2].poker.length == 17){
						let animate_1 = '';
						animate_1 = '<div class="animate_111" style="background:url(./images/ct.gif) no-repeat;width:1920px;height:1080px;position:absolute;top:0px;left:0px;z-index:2"><button class="again_1">再玩一局</button></div>';
		...
		setTimeout(function(){
			for(let i=0;i<player[index].poker.length;i++){
				if(index == 1){
					...
					$('.play').eq(index).css({left:i+'px','top':6*i+'px'});
				}else{
					poker_html = makePoker(player[index].poker[i]);
					$('.play').eq(index).append(poker_html);
					$('.play').eq(index).find('li:last').css({top:-24*i+'px','transform':'rotateZ(90deg)'});	
					$('.play').eq(index).css({left:i+'px'});
				}
	...
	function desktopPoker(index){
		...
		for(let i=0;i<game.select_poker.poker.length;i++){
			if(index == 1){	
				...
				$('.desktopPoke').eq(index).append(poker_html);
				$('.desktopPoke').eq(index).find('li:last').css({left:24*i+'px'});	
				$('.desktopPoke').eq(index).css({top:'300px'});
			}else{
				poker_html = makePoker(game.select_poker.poker[i]);
				$('.desktopPoke').eq(index).append(poker_html);
				$('.desktopPoke').eq(index).find('li:last').css({left:-24*i+'px'});	
	...
	function playcardAudio(type,num){
		switch(type){
			case 1:
				$('audio').attr('src','./audio/'+num+'.mp3');
			break;
			case 2:
				$('audio').attr('src','./audio/'+num+'_'+num+'.mp3');
			break;
			case 3:
				$('audio').attr('src','./audio/'+num+'_'+num+'_'+num+'.mp3');
			break;
		}
	}
	...
	function checkStraight(poker){
		...
		let animate_1 = '';
		animate_1 = '<div class="animate_11" style="background:url(./images/sz.gif) no-repeat;width:645px;height:258px;position:absolute;top:300px;left:630px;z-index:2"></div>';
	...
	function checkStraightPairs(poker){
		...
		$('audio').attr('src','./audio/888.mp3');
		let animate_1 = '';
		animate_1 = '<div class="animate_11" style="background:url(./images/ld.gif) no-repeat;width:800px;height:315px;position:absolute;top:215px;left:650px;z-index:2"></div>';
		$('body').append(animate_1);
		setTimeout(function(){
			$('.animate_11').remove();
		},1200);
	...
	function planePoker(poker){
		if(poker.length = 8){
			for(let i=0;i<poker.length-7;i++){
				if((poker[i+2].num != poker[i+4].num || poker[i+5].num != poker[i+7].num || poker[i+5].num*1 - 1 != poker[i+6].num) &&
					(poker[i+1].num != poker[i+3].num || poker[i+4].num != poker[i+6].num || poker[i+3].num*1 - 1 != poker[i+4].num) &&
					(poker[i].num != poker[i+2].num || poker[i+3].num != poker[i+5].num || poker[i+2].num*1 - 1 != poker[i+3].num)
					){
					return false;
				}
			}
		}
		$('audio').attr('src','./audio/男子飞机.mp3');
		let animate_1 = '';
		animate_1 = '<div class="animate_12" style="background:url(./images/fj.gif) no-repeat;width:680px;height:502px;position:absolute;top:85px;left:575px;z-index:2"></div>';
		$('body').append(animate_1);
		setTimeout(function(){
			$('.animate_12').remove();
		},2100);
